...
# -*- coding: utf-8 -*-
"""
Goal Consistency JSON数据转SQL INSERT语句脚本
用于将转换后的Goal Consistency JSON数据生成SQL INSERT语句
"""
...
def generate_insert_statement(record: Dict[str, Any], table_name: str = "goal_consistency_evaluations") -> str:
    """
    为单条记录生成SQL INSERT语句
...
def generate_batch_insert_statements(data: List[Dict[str, Any]], 
                                   table_name: str = "goal_consistency_evaluations",
                                   batch_size: int = 100) -> List[str]:
    """
    生成批量INSERT语句（每条记录一个INSERT）
...
def save_sql_file(sql_statements: List[str], output_path: str) -> None:
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as file:
            # 写入SQL文件头部注释
            file.write("-- ===============================================\n")
            file.write("-- Goal Consistency 批量插入SQL语句文件\n")
            ...
            file.write("CREATE TABLE IF NOT EXISTS goal_consistency_evaluations (\n")
...
def main():
    """
    主函数 - 执行Goal Consistency JSON到SQL转换流程
    """
    print("🚀 开始将Goal Consistency JSON数据转换为SQL INSERT语句")
    print("="*60)
    
    # 配置文件路径
    INPUT_FILE = "zhang-junjie-goalconsistency-transformed.json"
    OUTPUT_FILE_SINGLE = "goalconsistency_insert_statements_single.sql"
    OUTPUT_FILE_BATCH = "goalconsistency_insert_statements_batch.sql"
    TABLE_NAME = "goal_consistency_evaluations"
    ...
    try:
        ...
        print("\n" + "="*60)
        print("🎉 Goal Consistency JSON到SQL转换完成!")
